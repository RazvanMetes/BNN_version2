...
def binary_mnist(input, training=True):
	fc1 = layers.binaryDense(input, 32, activation=None, name="binarydense1", binarize_input=False)
	bn1 = tf.layers.batch_normalization(fc1, training=training)
	ac1 = tf.clip_by_value(bn1, -1, 1)
	fc2 = layers.binaryDense(ac1, 32, activation=None, name="binarydense2")
	bn2 = tf.layers.batch_normalization(fc2, training=training)
	ac2 = tf.clip_by_value(bn2, -1, 1)
	fc3 = layers.binaryDense(ac2, 32, activation=None, name="binarydense3")
